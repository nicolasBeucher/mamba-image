...
class TestHierarDualBld(unittest.TestCase):
    ...
    def testHierarDualBuild32(self):
        ...
        for i in range(10):
            vi =  random.randint(0,0x100000)
        
            self.im32_4.fill(0xffffffff)
            self._drawTestIm(self.im32_4, vi)
        
            self.im32_2.fill(0xffffffff)
            self.im32_2.setPixel(vi, (1,1))
            hierarDualBuild(self.im32_1, self.im32_2, grid=HEXAGONAL)
            (x,y) = compare(self.im32_4, self.im32_2, self.im32_3)
            self.assertLess(x, 0)
            
            self.im32_2.fill(0xffffffff)
            self.im32_2.setPixel(vi, (1,1))
            hierarDualBuild(self.im32_1, self.im32_2, grid=SQUARE)
            (x,y) = compare(self.im32_4, self.im32_2, self.im32_3)
            self.assertLess(x, 0)
